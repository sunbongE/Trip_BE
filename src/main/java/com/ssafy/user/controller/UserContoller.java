...
public class UserContoller {
	...
	public UserContoller(UserService userService, JWTUtil jwtUtil, ServletContext servletContext) {
...
//	public ResponseEntity<?> delete(@PathVariable("userId") String userId) throws SQLException{
	...
	@ApiOperation(value = "프로필 이미지 사진 등록하기", notes = "회원의 프로플 사진을 등록한다.")
	@PostMapping("/profile/regist")
	protected ResponseEntity<?> registerProfile(@RequestPart("userId") Map<String, String> userId,
...
//				String realPath = servletContext.getRealPath("/resources/img");
				...
				if (!folder.exists())
					folder.mkdirs();
				List<ProfileInfoDto> dtos = new ArrayList<ProfileInfoDto>();
	...
	@GetMapping("/profile/findProfileByUserId/{userId}")
	protected ResponseEntity<?> findProfileByUserId(@PathVariable String userId) {
		try {
			ProfileInfoDto dto = userService.findProfileByUserId(userId);
			if (dto == null) {
				return new ResponseEntity<Void>(HttpStatus.NOT_FOUND);
			} else {
	...
	/*
	 * deleteByUserId(String userId) findProfileByUserId(String userId)
	 */
